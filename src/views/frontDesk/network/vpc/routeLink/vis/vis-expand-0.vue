...
function buildOptions() {
    return {
        ...
        edges: {
            color: { color: '#666666', highlight: '#2b7ce9', opacity: 0.8 },
            width: 2,
            selectionWidth: 3,
...
function draw() {
    ...
    network.once('stabilizationIterationsDone', () => {
        ...
        // 优化显示：充分利用可用空间，减少左右空白
        setTimeout(() => {
            const containerRect = container.value.getBoundingClientRect();
            const networkRect = network.getBoundingBox();
            
            if (networkRect && containerRect.width > 0) {
                const containerWidth = containerRect.width;
                const containerHeight = containerRect.height;
                const networkWidth = networkRect.right - networkRect.left;
                const networkHeight = networkRect.bottom - networkRect.top;
                
                if (networkWidth > 0 && networkHeight > 0) {
                    const scaleX = (containerWidth * 0.9) / networkWidth;
