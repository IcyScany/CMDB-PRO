...
import API from "@/api/network/vpcPeering";
    ...
    >
        ...
        <template #id="{ row }">
            <a-tooltip placement="topLeft">
                <template #title>
                    <a-typography-paragraph :copyable="{ tooltip: false}" class="tooltip-text">{{ row.peering_id }}</a-typography-paragraph>
                </template>
                <span class="cursor-pointer text-primary" @click="handleSubPageOpen(row)">{{ row.id }}</span>
            </a-tooltip>
        </template>

        <template #peering_id="{ row }">
            <span class="cursor-pointer text-primary" @click="handleSubPageOpen(row)">{{ row.peering_id }}</span>
        </template>
        ...
        <template #peering_status="{row}">
            <vxe-tag :class="['活动的', '已激活'].includes(row.peering_status) ? 'theme--primary' : 'theme--error'" size="small">
                {{ row.peering_status}}
            </vxe-tag>  
        </template>

        <template #accept_tenant_id="{row}">
            <a-tooltip :title="row.request_tenant_id">
                <div class="truncate">
                    {{ row.request_tenant_id }}
                </div>
            </a-tooltip>
            <a-tooltip :title="row.accept_tenant_id">
                <div class="truncate">
                    {{ row.accept_tenant_id }}
                </div>
            </a-tooltip>
        </template>

        <template #accept_vpc_id="{row}">
            <a-tooltip :title="row.request_vpc_id">
                <div class="truncate">
                    {{ row.request_vpc_id }}
                </div>
            </a-tooltip>
            <a-tooltip :title="row.accept_vpc_id">
                <div class="truncate">
                    {{ row.accept_vpc_id }}
                </div>
            </a-tooltip>
