...
const formatData = async () => {
    ...
    data.section1.forEach(item => {
        // 处理status, 转中文名
        if(item.fields?.[0] === 'status') {
            let index_0 = item.items.findIndex(i => i.status === 0);
            let index_2 = item.items.findIndex(i => i.status === 2);
            item.items[index_0].count = item.items[index_0].count + item.items[index_2].count || 0;
            item.items.splice(index_2, 1);
    ...
    data.section2.forEach(item => {
        // 处理regions_id, 转中文名
        if(item.fields?.[0] === 'regions_id') {
            item.items.forEach(data => {
                if(data.regions_id) {
                    data.regions_id = data.regions_id.split('、').map(i => state.regionList?.[i] || i).join('，');
                } else {
                    data.regions_id = '其他';
                }
            });
    ...
    <div v-else class="overview-container">
        ...
        <div class="summary-box">
            <div 
                ...
                @click="index === 0 ? $router.push({ name: 'extranetDomainPage' }) : null"
            >
