...
const summaryMeta = {
    total: { label: '总数', color: '#4fb050', icon: 'kvstore.png', pointer: true, name: 'networkHardwarePage' },
...
const formatData = async () => {
    ...
    // 处理typed
    let typedData = data.typed;
    typedData.forEach(item => {
        item.typed = item.typed + ' ' + item.brand;
    });

    if(typedData.length > 5) {
        data.typed = typedData.slice(0, 4);
        data.typed.push({
            typed: '其他',
            count: typedData.slice(4).reduce((acc, item) => acc + item.count, 0),
            other_data: typedData.slice(4),
        });
    }

    // 处理machine_room
    let machineRoomData = data.machine_room;
    if(machineRoomData.length > 5) {
        data.machine_room = machineRoomData.slice(0, 4);
        data.machine_room.push({
            machine_room: '其他',
            count: machineRoomData.slice(4).reduce((acc, item) => acc + item.count, 0),
            other_data: machineRoomData.slice(4),
        });
    }
