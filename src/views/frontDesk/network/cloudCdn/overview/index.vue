...
const summaryMeta = {
    total: { label: '总数', color: '#4fb050', icon: 'kvstore.png', pointer: true, name: 'cloudCdnPage' },
    domain_status: { label: '域名状态', color: '#e88250' },
};

const overviewMeta = {
    environment: { label: '环境分布' },
    cloud_source: { label: '源站分布' },
    https_status: { label: '是否开启HTTPS加速' },
    enterprise_project: { label: '企业项目' },
};
...
const formatData = async () => {
    ...
    // 处理https_status
    data.https_status.forEach(item => {
        item.https_status = {
            0: '未开启',
            1: '已开启',
        }[item.https_status] || item.https_status;
    });
