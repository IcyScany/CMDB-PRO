<script setup>
import API from '@/api/monitor/certManage';
...
const summaryMeta = {
    total: { label: '总数', color: '#4fb050', icon: 'kvstore.png', pointer: true, name: 'certificateManagementPage' },
...
const getData = async () => {
    ...
    // 处理状态
    state.overviewData.status.forEach(item => {
        item.status = item.status === 1 ? '有效证书' : '无效证书';
    });
};
...
<template>
    <div v-if="state?.overviewData?.total?.[0]?.count > 0" class="overview-container">
        <!-- 统计卡片 -->
