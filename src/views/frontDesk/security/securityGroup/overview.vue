<script setup>
import securityGroupApi from '@/api/security/securityGroup';
...
onMounted(async () => {
    try {
        const res = await securityGroupApi.getOverview();
        state.value = res || {};
        console.log(res);
        await formatData();
    } catch (error) {
        ...
        <div v-if="section1.length > 0" class="overview-container">
            ...
            <div class="summary-box">
                <div
                    ...
                    @click="key === '总数' ? $router.push({ name: 'securityGroupList' }) : null"
                >
                    <div 
