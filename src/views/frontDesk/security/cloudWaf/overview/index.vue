...
const summaryMeta = {
    total: { label: '总数', color: '#4fb050', icon: 'kvstore.png', pointer: true, name: 'cloudWafPage' },
    access_status: { label: '接入状态', color: '#3c79bf' },
    protect_status: { label: '防护状态', color: '#e88250' },
};

const overviewMeta = {
    environment: { label: '环境分布' },
    cloud_source: { label: '云来源分布' },
...
const getData = async () => {
    ...
    state.cloudRegionList = cloudRegionList;
...
const formatData = async () => {
    ...
    // 处理access_status
    data.access_status.forEach(item => {
        item.access_status = {
            0: '未接入',
            1: '接入',
        }[item.access_status] || item.access_status;
    });

    // 处理protect_status
    data.protect_status.forEach(item => {
        item.protect_status = {
            0: '暂停防护',
            1: '开启防护',
        }[item.protect_status] || item.protect_status;
    });
    ...
    data.region_id.forEach(item => {
        item.region_id = state.cloudRegionList[item.region_id];
    });
