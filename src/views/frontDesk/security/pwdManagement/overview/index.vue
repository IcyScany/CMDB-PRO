...
const summaryMeta = {
    total: { label: '总数', color: '#4fb050', icon: 'kvstore.png', pointer: true, name: 'pwdManagementPage' },
};

const overviewMeta = {
    use_limits: { label: '使用限制' },
    types: { label: '类型' },
    share_limits: { label: '共享限制' },
};
...
const formatData = async () => {
    ...
    let typeMap = {
        "Token": "Token",
        "Certificate": "证书",
        "Secret file": "密钥SecretKey",
        "Secret text": "密码Password",
        "Username with password": "用户名Username+密码Password",
        "AccessKey+SecretAccessKey": "AccessKey+SecretAccessKey",
        "SSH Username with private key": "用户名Username+私钥PrivateKey"
    };
    data.types.forEach(item => {
        item.types = typeMap[item.types] || item.types;
    });

    // 处理share_limits
    data.share_limits.forEach(item => {
        item.share_limits = item.share_limits.includes('ALL') ? '公共' : '其他';
    });
