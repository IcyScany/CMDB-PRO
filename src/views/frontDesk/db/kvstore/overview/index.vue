...
const getData = async () => {
    ...
    // 磁盘使用量Top10
    state.overviewData.mem_usage_rate_top_n = res.mem_usage_rate_top_n;
    state.overviewData.mem_usage_rate_top_n.forEach(item => {
        item.mem = Number(item.mem / 1024).toFixed(1);
        item.mem_used = Number(item.mem_used / 1024).toFixed(1);
    });
};
...
const renderVolumeChart = () => {
    if (!state.overviewData.mem_usage_rate_top_n) return;
    // 按使用率降序排列
    const data = [...state.overviewData.mem_usage_rate_top_n].sort(
        (b, a) => b.mem_usage_rate - a.mem_usage_rate
    );
        ...
        tooltip: {
            ...
            formatter: function(params) {
                ...
                return `
                    ...
                    环境：${d.environment}<br/>
                    总容量：${d.mem} GB<br/>
                    已用容量：${d.mem_used} GB<br/>
                    使用率：${d.mem_usage_rate}%
                `;
            }
        },
        grid: {
        ...
        series: [
            {
                ...
                label: {
                    ...
                    formatter: function(params) {
                        ...
                        return `${d.mem_usage_rate}%  (${d.mem_used}/${d.mem}GB)`;
                    }
                }
            }
        ]
    ...
    <div v-else class="overview-container">
        ...
        <div class="summary-box">
            <div 
                ...
                @click="item.pointer ? $router.push({ name: 'kvstorePage' }) : null"
            >
