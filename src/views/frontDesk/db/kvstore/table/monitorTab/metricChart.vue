...
import { kvstoreMetrics } from './config';
...
const groupName = 'kvstore-metric-group';

// 获取图表数据
const getChartData = async () => {
    const { metrics } = await apiList.monitor.postChartData(props.sid, props.param);
...
const renderChart = () => {
    ...
    const unit = kvstoreMetrics[props.metricName]?.unit || state.chartData.unit || '';
    const label = kvstoreMetrics[props.metricName]?.label || props.metricName;
            ...
            // 圆圈问号图标
            {
                ...
                children: [
                    ...
                    // 问号
                    {
                        ...
                        tooltip: {
                            show: true,
                            formatter: kvstoreMetrics[props.metricName]?.tooltip,
                            backgroundColor: '#fff',
                            borderColor: '#e8e8e8',
                            borderWidth: 1,
                            textStyle: {
