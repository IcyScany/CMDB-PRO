    ...
    <div v-for="item in blockList" :key="item.title" class="mb-4">
        ...
        <a-card :style="{borderColor: '#cccccc'}">
            ...
            >
                ...
                <template #addresses="{ row }">
                    ...
                    >
                        <a-tooltip :overlay-style="{ fontSize: '12px', maxWidth: '400px' }">
                            ...
                            <a-typography-paragraph :copyable="{ tooltip: false}" class="copy-text inline-block">
                ...
                <template #mem="{row}">
                    <div>{{ Number(row.mem / 1024).toFixed(1) }}GB</div>
                </template>

                <template #mem_used="{row}">
                    <div>{{ Number(row.mem_used / 1024).toFixed(1) }}GB</div>
                </template>
            </ops-descriptions>
