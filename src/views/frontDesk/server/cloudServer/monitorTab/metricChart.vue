...
const groupName = 'ecs-metric-group';

// 获取图表数据
const getChartData = async () => {
    try {
        const { data } = await cloudServerApi.monitorTabApi.postMonitorTabDetail({
            server_id: props.sid,
            data: props.param
        });
        state.chartData = data || { values: [] };
...
const renderChart = () => {
    ...
    const unit = ecsMetrics[props.metricName]?.unit || state.chartData.unit || '';
    const label = ecsMetrics[props.metricName]?.label || props.metricName;
            ...
            // 圆圈问号图标
            {
                ...
                children: [
                    ...
                    // 问号
                    {
                        ...
                        tooltip: {
                            show: true,
                            formatter: ecsMetrics[props.metricName]?.tooltip,
                            backgroundColor: '#fff',
                            borderColor: '#e8e8e8',
                            borderWidth: 1,
                            textStyle: {
