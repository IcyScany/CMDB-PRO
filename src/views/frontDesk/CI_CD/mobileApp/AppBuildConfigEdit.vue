...
const {postAddBuildConfig, putBuildConfigEdit, getBuildConfigList } = mobileAppApi;
const route = useRoute();
const customEditData = () => {

};
const customSubmitData = async (data) => {
    data.app_id = route?.query?.appId ? Number(route.query.appId) : null;
...
let {
    ...
    formDataInit,
    editSubmit,
} = opsFormInit({formRef, editTitleLayer, formType, sid,emits,addApi: postAddBuildConfig, editApi: putBuildConfigEdit, oneDataApi: getBuildConfigList , customSubmitData, customEditData});
...
<template>
    <ops-form-container
        :title="`构建配置${formType === $CONFIG.FORM_TYPE.ADD_TYPE ? '新增': '编辑'}`"
        :open="open"
