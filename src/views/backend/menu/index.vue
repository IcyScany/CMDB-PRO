...
const getPrimaryMenu = async () => {
    ...
    try {
        ...
        state.leftMenuLevel = requestData;
...
const rowDrop = () => {
    sortableMenuLevel = Sortable.create(document.getElementById('menu-level1-box'), {
        handle: '.drag-menu-level1-btn',
        onEnd: (sortableEvent) => {
            const newIndex = sortableEvent.newIndex;
            const oldIndex = sortableEvent.oldIndex;
            const currRow = state.leftMenuLevel.splice(oldIndex, 1)[0];
            state.leftMenuLevel.splice(newIndex, 0, currRow);
            const dataRowIndex = findKey(state.leftMenuLevel, item => item.id === currRow.id); // 获取数据相对data数据的位置

            menusApi.putOneMenuSort(currRow.id, dataRowIndex).then((res) => {
...
<template>
    <a-layout style="height: 100%">
        <a-layout-sider v-model:collapsed="state.collapsed" collapsible theme="light" collapsed-width="30">
            <a-spin :spinning="state.leftMenuLoading">
                <a-menu id="menu-level1-box" ref="menuBox" v-model:selectedKeys="state.selectedLeftKeys"  theme="light" mode="inline" @select="({key, item}) => handleTableData(key, item)" >
                    <template v-if="state.leftMenuLevel.length">
                        <a-menu-item v-for="level_1 in state.leftMenuLevel" :key="level_1.id" :item="level_1">
                        <span class="drag-menu-level1-btn cursor-move">
                            <i class="vxe-table-icon-drag-handle"></i>
                        </span>
                        <span>
                            <svg-icon :iconname="level_1.icon"/> {{level_1.menu_name}}
                        </span>
                        <span style="float: right">
                            <vxe-button mode="text" icon="vxe-icon-edit" status="primary" @click.stop="handleUserEdit({type:$CONFIG.FORM_TYPE.EDIT_TYPE, row: level_1 })"/>
                        </span>
                        </a-menu-item>
                    </template>
            ...
            <a-layout-content>
                ...
                <ops-table
                    ...
                    :request-config="{
                        ...
                        params: state.selectedLeftKeys[0],
                        immediate: false,
                    }"
