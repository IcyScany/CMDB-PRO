...
const cmdbFormInit = ({
    ...
    /**
     * 提交编辑接口
     * @param isConfigHeader 提交接口需要传递请求头的配置
     *
     * **/
    const submit = async (data, isConfigHeader) => {
        let params = '';
        if (formType.value === ADD_TYPE) { // 新增时
            ...
            const addHttp = await  http.post(`${urlPrefix}${addUrl}${params}`, data, isConfigHeader);
            addHttp.then(res => {
                confirmLoading.value = false;
            ...
            if (Array.isArray(sid.value)) { // 批量处理
     ...
     * 编辑情况时，初始化表单值
     */
    const formDataEditInit = () => {
    // 业务模块单独处理获取到单条数据（比如对edit_data某些字段特殊处理）
        if (customEditData) {
            customEditData(edit_data);
        }
        // 填充edit_data的值到formState
        geneEditData({edit_title: title.value, edit_data: edit_data.value, formState});
